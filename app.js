...
App({
  onLaunch: () => {
    ...
    console.log(`用户当前存储在本地的第三方 session_key（local_session_key）: %c${local_session_key}`,`color: #ffff; background-color: #113c6d;`);
    ...
    wx.login({
      ...
      success: res => {
        ...
        console.log(`使用 we.login 从微信拿到用户的code: %c${res.code}`,`color: #ffff; background-color: #113c6d;`);
        ...
        if (res.code) {
          ...
          wx.request({
            ...
            success: res => {
              ...
              console.log(`第三方返回重组完成的 session_key: %c${res.data.session_key}`,`color: #ffff; background-color: #113c6d;`);
              if (res.data.session_key) {
                ...
                try {
                  ...
                  const local_session_key = wx.getStorageSync(`local_session_key`);
                  console.log(`执行 wx.setStorageSync 成功，local_session_key 变更为: %c${local_session_key}`,`color: #ffff; background-color: #113c6d;`);
                }
                catch (e) {
                ...
                wx.getSetting({
                  ...
                  success: res => {
                    if (!res.authSetting['scope.userInfo']) {
                      console.log(`用户并未授权获取TA的公开信息，权限属性 scope.userInfo: %c${res.authSetting[`scope.userInfo`]}`,`color: #ffff; background-color: #113c6d;`);
                      // 获取用户授权拿TA的公开信息
                      wx.getUserInfo({
                        fail: res => {
                        ...
                        success: res => {
                          ...
                          wx.getSetting({
                            success: res => {
                              console.log(`好了，用户授权获取TA的公开信息，权限属性 scope.userInfo: %c${res.authSetting[`scope.userInfo`]}`,`color: #ffff; background-color: #113c6d;`);
                            }
                          });
                      ...
                      console.log(`用户已经授权获取TA的公开信息，权限属性 scope.userInfo: %c${res.authSetting[`scope.userInfo`]}`,`color: #ffff; background-color: #113c6d;`);
                    }
                  }
                })
              }
